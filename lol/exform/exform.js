(function($){
 ...
 jQuery.fn.exform = function(options){
  ...
  $('head').append('<link rel="stylesheet" type="text/css" href="'+ options.path +'themes/'+ options.theme +'/css/'+ options.cssfile +'" />');
  ...
  function show_form(data){ 
   if(exform_blck == 'body'){
	...
	$(exform_wrpr).css({position: 'fixed', left: '50%', top: '50%',
		'margin-left': '-' + parseInt($('.exform_wrapper.'+options.theme).css('width')) / 2 + 'px',
		'margin-top': '-' + parseInt($('.exform_wrapper.'+options.theme).css('height')) / 2 + 'px'
	}).show();
   }
   else{
   ...
   if($(exform_wrpr +'.final_message div').length > 0) setTimeout(remove_form, 4000)
  }
   ...
   if($(exform_wrpr +' input[name="user_name"]').val().length == 0){
	...
	$.post(options.path + 'exform.php', $(exform_wrpr +' form[name="exform"]').serialize(), function(data){
	 if(options.wrapper == 'body'){
	  ...
	  $(exform_wrpr +' .mf_submit input[type="button"]').remove();
	  operations();
	  $(exform_wrpr +'.final_message').fadeIn(500).delay(1000).fadeOut(1000, function(){ location.reload() });
	 }
	})
   } 
   else remove_form();
